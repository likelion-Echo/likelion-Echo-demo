...
1. 회원가입 → 로그인
2. **나의 기록**에 편지·일기·음성 등록 (음성은 업로드 후 자동으로 텍스트 변환)
3. **가치관** 질문 6개 답변
4. **페르소나** 생성 — GPT가 기록을 분석해 말투·가치관 구조화
5. **미래 메시지**에서 "취업 성공" 이벤트 생성 + 기록 연결 → **초대 코드**가 발급된다
6. 초대 코드를 받을 사람에게 전달

**수신자**

7. 다른 계정으로 로그인 → **수신함**에 초대 코드 입력 → 이벤트 연결
8. 이벤트 활성화 → 작성자가 남긴 편지·음성 확인
9. **Echo와 대화** — "아빠 나 취업했어요" → 작성자의 기록에서 관련 내용을 찾아 답변 + 근거 기록 표시
...
Voice Clone, 영상 업로드, 사진 OCR, 자동 사망 확인, 감정 보호 기능.
